...
import { DollarSign, ArrowDownCircle, Banknote, X } from 'lucide-react';

export default function Ahorros() {
  const userId = useStore((state) => state.userId);
  const { ahorros, totals, loading } = useFinanceData();
  
  const [modalType, setModalType] = useState(null); // 'cargar_ars', 'comprar_usd', 'retirar'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', position: 'relative' }}>
      ...
      {modalType && (
        <Modal title={modalType === 'cargar_ars' ? 'Cargar Ahorros (ARS)' : modalType === 'retirar' ? 'Retirar Fondos (ARS)' : 'Comprar Dólares'} onClose={closeModal}>
          {modalType === 'comprar_usd' ? (
            ...
            <form onSubmit={handleAction}>
              ...
              <input className="input-field" type="text" placeholder={modalType === 'cargar_ars' ? "Ej: Regalo" : "Ej: Compra compu"} value={desc} onChange={e => setDesc(e.target.value)} />
